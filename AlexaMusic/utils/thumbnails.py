...
async def gen_thumb(videoid, user_id, theme):
    ...
    try:
        ...
        try:
            text_w, text_h = draw.textsize(f"Mamakklı ya teşekkürler", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"mamaklıya teşekkürler",
                fill="red",
                font=font,
            )
            ...
            pass
        text_w, text_h = draw.textsize(f"YouTube: yoodelidegilim", font=arial)
        draw.text(
            ((1280 - text_w) / 2, 620),
            f"YouTube: yoodelidegilim",
            fill="white",
            font=arial,
        )
...
async def gen_qthumb(videoid, user_id, theme):
    ...
    try:
        ...
        para = textwrap.wrap(title, width=32)
        try:
            text_w, text_h = draw.textsize(f"MAMAKLİ ADDED THIS SONG TO QUEUE", font=font)
            draw.text(
                ((1280 - text_w) / 2, 30),
                f"MAMAKLİ ADDED THIS SONG TO QUEUE",
                fill="red",
                font=font,
            )
            text_w, text_h = draw.textsize(
                f"Mamakli Music One Of The Most Advanced Telegram Music Bot", font=arial
            )
            draw.text(
                ((1280 - text_w) / 2, 80),
                f"Mamakli Music One Of The Most Advanced Telegram Music Bot",
                fill="green",
                font=arial,
            )
            if para[0]:
